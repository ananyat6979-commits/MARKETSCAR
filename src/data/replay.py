...
class ReplayEngine:
    ...
    def __init__(
        ...
        print(
            f"Date range: {self.df['InvoiceDate'].min()} to {self.df['InvoiceDate'].max()}"
        )
    ...
    def _normalize_manifest_hash(self, manifest):
        # support multiple possible field names, return lowercased hex or None
        file_info = manifest.get("file", {})
        for key in ("hash", "sha256", "Sha256", "Hash"):
            if key in file_info:
                return file_info[key].lower()
        return None
    ...
    def _verify_integrity(self):
        ...
        # Get expected hash (normalized)
        expected_hash = self._normalize_manifest_hash(manifest)
        ...
        actual_hash = computed_hash.lower()

        # Validate presence
        if expected_hash is None:
            raise ValueError(
                "Manifest missing expected hash field ('hash'/'sha256'/'Sha256'/'Hash')"
            )
    ...
    def get_window(
        ...
        mask = (self.df["InvoiceDate"] >= start_date) & (
                ...
                synthetic_txns.append(
                    {
                        ...
                        + timedelta(seconds=i * 0.04),  # Clustered timestamps
...
def main():
    ...
    for txn in engine.stream(max_events=10, realtime=False):
        print(
            f"{txn.invoice_date} | {txn.stock_code:8s} | £{txn.price:6.2f} x {txn.quantity:2d} = £{txn.revenue:7.2f}"
        )
        count += 1
