...
def parse_args():
    ...
    p.add_argument("--n-bins", type=int, default=128, help="Bins used by histogram fallback / grid size")
    p.add_argument("--bw", type=float, default=None, help="bandwidth for gaussian_kde if available")
    p.add_argument("--n-bootstrap", type=int, default=200, help="Bootstrap iterations")
    p.add_argument("--sample-size", type=int, default=None, help="Sample size per bootstrap (None => full length)")
    p.add_argument("--price-col", type=str, default=None, help="Price column name (auto-detected if not set)")
    return p.parse_args()
...
def _detect_price_column(df: pd.DataFrame, explicit: str | None) -> str:
    if explicit:
        if explicit in df.columns:
            return explicit
        raise KeyError(f"Requested price column '{explicit}' not present. Available: {list(df.columns)}")
    # common candidates
    candidates = ["price", "Price", "unit_price", "UnitPrice", "Unit Price", "Unit price", "Unit_Price"]
    for c in candidates:
        if c in df.columns:
            return c
    ...
    raise KeyError(f"Could not auto-detect price column. Candidates: {candidates}. Available columns: {list(df.columns)}")
...
def main():
    ...
    print(f"seed={args.seed} price_col={price_col} sample_size={len(df)} p95={p95:.6f} p99={p99:.6f}")
